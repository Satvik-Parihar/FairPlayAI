        ...
        <div className={`grid ${metrics.regression_fairness && Object.keys(metrics.regression_fairness).length === 1 ? 'grid-cols-1' : 'lg:grid-cols-2 md:grid-cols-1'} gap-6 mb-8`}>
         ...
         (
          <div className="grid lg:grid-cols-2 gap-6 mb-8">
            ...
            <Card className="h-full flex flex-col max-h-[420px]">
              <CardHeader>
                <CardTitle className="text-blue-700">Demographic Parity</CardTitle>
                <CardDescription>Equal positive prediction rates across groups</CardDescription>
              </CardHeader>
              <CardContent className="flex-1 overflow-y-auto">
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm text-left">
                    ...
                    <tbody>
                      {metrics.demographic_parity && typeof metrics.demographic_parity === "object" && Object.keys(metrics.demographic_parity).length > 0 ? (
                        Object.entries(metrics.demographic_parity).map(([attr, val]) => (
                          <tr key={attr} className="border-b last:border-b-0">
                            <td className="px-3 py-2 font-mono text-gray-700">{attr}</td>
                            <td className="px-3 py-2">
                              {typeof val === "number" && !isNaN(val) ? val.toFixed(4) : <span className="text-gray-400">N/A</span>}
                            </td>
                          </tr>
                        ))
                      ) : (
                        <tr><td colSpan={2} className="text-gray-400 px-3 py-2">N/A</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            <Card className="h-full flex flex-col max-h-[420px]">
              <CardHeader>
                <CardTitle className="text-blue-700">Equalized Odds</CardTitle>
                <CardDescription>Equal true/false positive rates across groups</CardDescription>
              </CardHeader>
              <CardContent className="flex-1 overflow-y-auto">
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm text-left">
            ...
            {/* Calibration Card */}
            <Card className="h-full flex flex-col max-h-[420px] lg:col-span-2">
              <CardHeader>
                <CardTitle className="text-blue-700">Calibration</CardTitle>
                <CardDescription>Predicted probabilities match actual outcomes</CardDescription>
              </CardHeader>
              <CardContent className="flex-1 overflow-y-auto">
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm text-left">
                      ...
                      {metrics.calibration && typeof metrics.calibration === "object" && Object.keys(metrics.calibration).length > 0 ? (
                        Object.entries(metrics.calibration).map(([attr, val]) => {
                          ...
                          return (
                            <tr key={attr} className="border-b last:border-b-0">
                              <td className="px-3 py-2 font-mono text-gray-700">{attr}</td>
                              <td className="px-3 py-2">{overallScore !== null && !isNaN(overallScore) ? overallScore.toFixed(4) : <span className="text-gray-400">N/A</span>}</td>
                              <td className="px-3 py-2">
                              ...
                              <td className="px-3 py-2 text-xs text-gray-600">{explanation}</td>
                            </tr>
                          );
                        })
                      ) : (
                        <tr><td colSpan={4} className="text-gray-400 px-3 py-2">N/A</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            {/* Individual Fairness Card */}
            <Card className="h-full flex flex-col max-h-[420px] lg:col-span-2">
              <CardHeader>
                <CardTitle className="text-blue-700">Individual Fairness</CardTitle>
                <CardDescription>Similar individuals receive similar outcomes</CardDescription>
              </CardHeader>
              <CardContent className="flex-1 overflow-y-auto">
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm text-left">
                    ...
                    <tbody>
                      {(() => {
                        ...
                        if (val === undefined || val === null || (typeof val === "number" && isNaN(val)) || (typeof val === "object" && Object.keys(val).length === 0)) {
                          return <tr><td colSpan={3} className="text-gray-400 px-3 py-2">N/A</td></tr>;
                        }
                        if (typeof val === "number") {
                          return (
                            <tr>
                              <td className="px-3 py-2 font-mono text-gray-700">Overall</td>
                              <td className="px-3 py-2">{val.toFixed(4)}</td>
                              <td className="px-3 py-2 text-xs text-gray-600">Higher values mean similar individuals receive similar outcomes.</td>
                            </tr>
                          );
                        }
                        if (typeof val === "object" && val !== null) {
                          return Object.entries(val).map(([group, groupVal]) => {
                            if (typeof groupVal === "number") {
                              return (
                                <tr key={group}>
                                  <td className="px-3 py-2 font-mono text-gray-700">{group}</td>
                                  <td className="px-3 py-2">{groupVal.toFixed(4)}</td>
                                  <td className="px-3 py-2 text-xs text-gray-600">Average similarity of predictions for individuals in <span className='font-mono'>{group}</span>.</td>
                                </tr>
                              );
                            }
                            if (typeof groupVal === "object" && groupVal !== null) {
                              const entries = Object.entries(groupVal);
                              const avg = entries.length > 0 ? (entries.reduce((a, [k, v]) => a + (typeof v === "number" && !isNaN(v) ? v : 0), 0) / entries.length) : null;
                              return (
                                <tr key={group}>
                                  <td className="px-3 py-2 font-mono text-gray-700">{group}</td>
                                  <td className="px-3 py-2">{avg !== null ? avg.toFixed(4) : <span className="text-gray-400">N/A</span>}</td>
                                  <td className="px-3 py-2 text-xs text-gray-600">Similarity scores for each value in <span className='font-mono'>{group}</span> (average shown).</td>
                                </tr>
                              );
                            }
                            return null;
